    ...
    <div class="space-y-6">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
                    ...
                    <table class="w-full text-left text-xs border-collapse">
                        <thead>
                            <tr class="bg-gray-50/50 dark:bg-gray-700/20 text-gray-400 dark:text-gray-500 font-bold border-b border-gray-100 dark:border-gray-700">
                                <th class="p-4 w-12">Urutan</th>
                                <th class="p-4">Tahapan / Milestone</th>
                                <th class="p-4">H- (Hari Sebelum Nikah)</th>
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-gray-700 text-gray-700 dark:text-gray-300 font-medium">
                            ...
                            <tr x-data="{ editing: false, title: '{{ $tpl->title }}', order: '{{ $tpl->order }}', days: '{{ $tpl->days_before_wedding }}', desc: '{{ $tpl->description }}' }" class="hover:bg-gray-50/40 dark:hover:bg-gray-700/10 transition-colors">
                                <!-- View mode -->
                                <td class="p-4 font-mono font-bold text-gray-400" x-show="!editing">#{{ $tpl->order }}</td>
                                <td class="p-4 font-bold text-gray-900 dark:text-white" x-show="!editing">{{ $tpl->title }}</td>
                                <td class="p-4 text-rose-600 dark:text-rose-400 font-semibold" x-show="!editing">H-{{ $tpl->days_before_wedding }} Hari</td>
                                <td class="p-4 text-gray-400 max-w-xs truncate" x-show="!editing" title="{{ $tpl->description }}">{{ $tpl->description ?? '-' }}</td>

                                <!-- Edit inline mode -->
                                <td colspan="4" class="p-4" x-show="editing">
                                    <form method="POST" action="{{ route('admin.timeline-templates.update', $tpl) }}" class="grid grid-cols-4 gap-2">
                                        @csrf
                                        @method('PUT')
                                        <input type="number" name="order" x-model="order" required class="w-16 bg-gray-50 dark:bg-gray-900 border border-gray-100 dark:border-gray-700 rounded-lg p-2 text-xs text-gray-900 dark:text-white focus:outline-none">
                                        <input type="text" name="title" x-model="title" required class="bg-gray-50 dark:bg-gray-900 border border-gray-100 dark:border-gray-700 rounded-lg p-2 text-xs text-gray-900 dark:text-white focus:outline-none">
                                        <input type="number" name="days_before_wedding" x-model="days" required class="w-24 bg-gray-50 dark:bg-gray-900 border border-gray-100 dark:border-gray-700 rounded-lg p-2 text-xs text-gray-900 dark:text-white focus:outline-none">
                                        ...
                                        <div class="col-span-4 flex justify-end gap-2 mt-1">
                                ...
                                <td class="p-4 text-center" x-show="!editing">
                                    <div class="flex items-center justify-center gap-2">
                                        ...
                                        <form method="POST" action="{{ route('admin.timeline-templates.destroy', $tpl) }}" class="inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="p-1.5 bg-red-50 dark:bg-red-950/20 hover:bg-red-100 text-red-600 dark:text-red-400 rounded-lg transition-colors" onclick="return confirm('Apakah anda yakin ingin menghapus template tahapan ini?')" title="Hapus">
                            ...
                            <tr>
                                <td colspan="5" class="p-8 text-center text-gray-400 dark:text-gray-500">
                                    Belum ada template timeline mileston terdaftar.
                                </td>
                            </tr>
                            @endforelse
                        </tbody>
            ...
            <div class="bg-white dark:bg-gray-800 p-6 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm space-y-4">
                <h3 class="font-bold text-sm text-gray-900 dark:text-white border-b border-gray-50 dark:border-gray-700 pb-3">Tambah Tahapan</h3>
                
                <form method="POST" action="{{ route('admin.timeline-templates.store') }}" class="space-y-4">
                    @csrf
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            <label class="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider block mb-1.5">H- (Hari)</label>
