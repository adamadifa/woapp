<x-wo-layout>
    <div class="space-y-6" x-data="{ 
        ...
        openEditModal(member, actionUrl) {
            this.editData.id = member.id;
            this.editData.name = member.name;
            this.editData.email = member.email;
            this.editData.sub_role = member.sub_role || 'assistant';
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                ...
                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Kelola hak akses akun staf dan asisten Wedding Organizer Anda.</p>
            </div>
            <div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @forelse($team as $member)
                <div class="bg-white dark:bg-gray-800 p-6 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm hover:shadow-md hover:-translate-y-1 transition-all duration-200 flex flex-col justify-between group">
                    <div class="space-y-4">
                        ...
                        <div class="flex items-start justify-between gap-4">
                            <div class="flex items-center gap-3">
                                <div class="w-10 h-10 rounded-xl bg-pink-50 dark:bg-pink-950/20 flex items-center justify-center text-pink-600 font-bold text-sm shrink-0">
                                    {{ substr($member->name, 0, 2) }}
                                </div>
                                <div>
                                    <h3 class="font-bold text-gray-900 dark:text-white group-hover:text-pink-500 transition-colors text-sm">{{ $member->name }}</h3>
                                    <span class="text-[9px] font-semibold text-gray-400 dark:text-gray-500">Staf WO</span>
                                </div>
                            </div>
                            
                            <span class="px-2 py-0.5 rounded text-[9px] font-bold uppercase tracking-wider 
                                @if($member->sub_role === 'admin') bg-pink-500 text-white
                                @elseif($member->sub_role === 'coordinator') bg-indigo-500 text-white
                                @else bg-gray-500 text-white @endif">
                                {{ strtoupper($member->sub_role ?? 'ASSISTANT') }}
                            </span>
                        </div>
                        ...
                        <div class="space-y-2 text-xs text-gray-500 dark:text-gray-400 pt-2">
                            <div class="flex items-center gap-2">
                                ...
                                <span class="font-mono text-[11px]">{{ $member->email }}</span>
                            </div>
                    ...
                    <div class="flex items-center justify-end gap-2 pt-5 mt-4 border-t border-gray-50 dark:border-gray-700/50">
                        <button @click="openEditModal({{ json_encode($member) }}, '{{ route('wo.team.update', $member) }}')" class="flex items-center gap-1.5 px-3 py-1.5 bg-gray-50 dark:bg-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 text-gray-600 dark:text-gray-300 text-xs font-semibold rounded-lg transition-colors" title="Edit Anggota">
                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                            <span>Edit</span>
                        </button>
                        
                        @if($member->id !== auth()->id())
                            <form method="POST" action="{{ route('wo.team.destroy', $member) }}" class="inline">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="flex items-center gap-1.5 px-3 py-1.5 bg-red-50 dark:bg-red-950/20 hover:bg-red-100 dark:hover:bg-red-900/30 text-red-600 dark:text-red-400 text-xs font-semibold rounded-lg transition-colors" onclick="return confirm('Apakah anda yakin ingin menghapus anggota tim ini?')" title="Hapus Anggota">
            ...
            @empty
                <div class="col-span-full bg-white dark:bg-gray-800 p-12 text-center rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm">
                    ...
                    <p class="text-sm font-medium text-gray-500 dark:text-gray-400">Belum ada anggota tim terdaftar.</p>
                </div>
            @endforelse
        </div>
            ...
            <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div class="inline-block align-bottom bg-white dark:bg-gray-800 rounded-3xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full border border-gray-100 dark:border-gray-700"
                    ...
                    <div class="px-6 py-5 border-b border-gray-50 dark:border-gray-700/50 flex items-center justify-between">
                        <h3 class="text-base font-bold text-gray-900 dark:text-white">Tambah Anggota Tim WO</h3>
                    ...
                    <form method="POST" action="{{ route('wo.team.store') }}" class="p-6 space-y-4">
                        @csrf
                        <div>
                            <label class="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider block mb-1.5">Nama Lengkap</label>
                            <input type="text" name="name" required placeholder="Contoh: Andi Wijaya" class="w-full bg-gray-50 dark:bg-gray-900 border border-gray-100 dark:border-gray-700 rounded-xl py-2.5 px-4 text-xs focus:ring-2 focus:ring-pink-500 focus:border-pink-500 focus:outline-none text-gray-900 dark:text-white transition-all">
                        </div>

                        <div>
                            <label class="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider block mb-1.5">Role / Jabatan Tim</label>
                            <select name="sub_role" required class="w-full bg-gray-50 dark:bg-gray-900 border border-gray-100 dark:border-gray-700 rounded-xl py-2.5 px-4 text-xs focus:ring-2 focus:ring-pink-500 focus:border-pink-500 focus:outline-none text-gray-900 dark:text-white transition-all">
                                <option value="assistant">Asisten WO</option>
                                <option value="coordinator">Koordinator Lapangan</option>
                                <option value="admin">Administrator Tim</option>
                            </select>
                        </div>

                        <div>
                            <label class="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider block mb-1.5">Email Akun</label>
                            <input type="email" name="email" required placeholder="Contoh: andi@gmail.com" class="w-full bg-gray-50 dark:bg-gray-900 border border-gray-100 dark:border-gray-700 rounded-xl py-2.5 px-4 text-xs focus:ring-2 focus:ring-pink-500 focus:border-pink-500 focus:outline-none text-gray-900 dark:text-white transition-all">
                        </div>

                        <div>
                            <label class="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider block mb-1.5">Password Login (Min. 6 Karakter)</label>
                            <input type="password" name="password" required placeholder="Password login..." class="w-full bg-gray-50 dark:bg-gray-900 border border-gray-100 dark:border-gray-700 rounded-xl py-2.5 px-4 text-xs focus:ring-2 focus:ring-pink-500 focus:border-pink-500 focus:outline-none text-gray-900 dark:text-white transition-all">
                        </div>
        ...
        <div x-show="showEditModal" 
            ...
            <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div class="inline-block align-bottom bg-white dark:bg-gray-800 rounded-3xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full border border-gray-100 dark:border-gray-700"
                    ...
                    <div class="px-6 py-5 border-b border-gray-50 dark:border-gray-700/50 flex items-center justify-between">
                        <h3 class="text-base font-bold text-gray-900 dark:text-white">Edit Anggota Tim</h3>
                    ...
                    <form method="POST" :action="editData.action" class="p-6 space-y-4">
                        ...
                        <div>
                            <label class="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider block mb-1.5">Role / Jabatan Tim</label>
                            <select name="sub_role" x-model="editData.sub_role" required class="w-full bg-gray-50 dark:bg-gray-900 border border-gray-100 dark:border-gray-700 rounded-xl py-2.5 px-4 text-xs focus:ring-2 focus:ring-pink-500 focus:border-pink-500 focus:outline-none text-gray-900 dark:text-white transition-all">
                                <option value="assistant">Asisten WO</option>
                                <option value="coordinator">Koordinator Lapangan</option>
                                <option value="admin">Administrator Tim</option>
                            </select>
                        ...
                        <div>
                            <label class="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider block mb-1.5">Password Login (Biarkan kosong jika tidak diubah)</label>
